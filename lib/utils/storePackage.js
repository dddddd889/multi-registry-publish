const { CACHE_NAME, PKG_NAME } = require('../../runtime')
const { publishConfigDefault } = require('../constants/pkgDefault')
...
exports.restorePackage = function restorePackageFn(packCache) {
  const { dataBuf } = packCache || {}
  if (existsSync(CACHE_NAME)) {
    unlinkSync(CACHE_NAME)
  }
  if (dataBuf) {
